...
function saveRawMaterial(name, price, stock) {
  let rawMaterials = loadData(STORAGE_KEYS.RAW_MATERIALS) || [];
  // Check if material exists, update if yes
  const index = rawMaterials.findIndex(m => m.name.toLowerCase() === name.toLowerCase());
  if (index >= 0) {
    rawMaterials[index].price = price;
    rawMaterials[index].stock = stock;
  } else {
...
function setupInputPage() {
  ...
  if (rawMaterialForm) {
    rawMaterialForm.addEventListener('submit', e => {
      ...
      saveRawMaterial(name, price, stock);
